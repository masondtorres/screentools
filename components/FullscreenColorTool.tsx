...
export function FullscreenColorTool({
  ...
  const [monitorPattern, setMonitorPattern] = useState<"solid" | "gradient" | "grid" | "text" | "backlight">("solid");
  ...
  return (
    <section className="tool-panel overflow-hidden rounded" aria-label={title}>
      <div
        ...
        onClick={() => isFullscreen && setControlsVisible((value) => !value)}
      >
      ...
      <div className="grid gap-5 p-4 sm:p-5">
        {mode === "monitor" ? (
          <div className="grid grid-cols-2 gap-2 sm:grid-cols-5">
            {(["solid", "gradient", "grid", "text", "backlight"] as const).map((pattern) => (
        ...
        {mode === "dead-pixel" ? (
          <div className="grid gap-3">
            <div className="flex flex-wrap gap-2">
              ...
              <button type="button" onClick={() => setAutoCycle(false)} className="min-h-11 rounded border border-line bg-white px-4 py-2 text-sm font-semibold">Stop auto-cycle</button>
            </div>
        ...
        <div className="flex flex-wrap gap-2">
          <button type="button" onClick={enterFullscreen} className="min-h-11 rounded bg-ink px-4 py-2 text-sm font-bold text-white">Go Full Screen</button>
        ...
        <p className="text-sm text-gray-600">Shortcuts: F fullscreen, Space controls, 1-9 preset colors, arrow keys cycle test colors.</p>
      </div>
    </section>
  );
}
...
function getStageStyle(color: string, brightness: number, mode: Mode, monitorPattern: string): React.CSSProperties {
